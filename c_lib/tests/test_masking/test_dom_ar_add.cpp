...
TEST_CASE("Second-order DOM 8bit arithmetic ADD gadget computes correctly", "[unittest][dom]") {
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        dom_ar_add8(masked_x, masked_y, out);

        const uint8_t expected_result = static_cast<uint8_t>(um_x + um_y);
...
TEST_CASE("Second-order DOM 32bit arithmetic ADD gadget computes correctly", "[unittest][dom]") {
    constexpr unsigned int iterations = 100;
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        dom_ar_add32(masked_x, masked_y, out);

        const uint32_t expected_result = static_cast<uint32_t>(um_x + um_y);
...
TEST_CASE("Second-order DOM 64bit arithmetic ADD gadget computes correctly", "[unittest][dom]") {
    constexpr unsigned int iterations = 100;
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        dom_ar_add64(masked_x, masked_y, out);

        const uint64_t expected_result = static_cast<uint64_t>(um_x + um_y);
