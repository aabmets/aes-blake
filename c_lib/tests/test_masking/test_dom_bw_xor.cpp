...
TEST_CASE("Second-order DOM XOR 8bit gadget computes correctly", "[unittest][dom]") {
    ...
    csprng_read_array(unmasked_y, iterations);
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        dom_bw_xor8(masked_x, masked_y, out);
...
TEST_CASE("Second-order DOM XOR 32bit gadget computes correctly", "[unittest][dom]") {
    constexpr unsigned int iterations = 100;
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        dom_bw_xor32(masked_x, masked_y, out);
...
TEST_CASE("Second-order DOM XOR 64bit gadget computes correctly", "[unittest][dom]") {
    constexpr unsigned int iterations = 100;
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        dom_bw_xor64(masked_x, masked_y, out);
