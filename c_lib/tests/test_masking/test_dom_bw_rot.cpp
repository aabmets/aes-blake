...
TEST_CASE("Second-order DOM ROTL 8bit gadget computes correctly", "[unittest][dom]") {
    constexpr unsigned int iterations = 100;
    const uint8_t bit_count = CHAR_BIT * sizeof(uint8_t);
    const uint8_t shift = 3;
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        uint8_t out[N_SHARES];
        dom_bw_rotl8(masked_x, out, shift);
...
TEST_CASE("Second-order DOM ROTL 32bit gadget computes correctly", "[unittest][dom]") {
    constexpr unsigned int iterations = 100;
    const uint8_t bit_count = CHAR_BIT * sizeof(uint32_t);
    const uint8_t shift = 17;
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        uint32_t out[N_SHARES];
        dom_bw_rotl32(masked_x, out, shift);
...
TEST_CASE("Second-order DOM ROTL 64bit gadget computes correctly", "[unittest][dom]") {
    constexpr unsigned int iterations = 100;
    const uint8_t bit_count = CHAR_BIT * sizeof(uint64_t);
    const uint8_t shift = 47;
    ...
    for (int i = 0; i < iterations; ++i) {
        ...
        uint64_t out[N_SHARES];
        dom_bw_rotl64(masked_x, out, shift);
