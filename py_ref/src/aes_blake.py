...
class AESBlake:
    ...
    def encrypt(self, plaintext: bytes, nonce: bytes, header: bytes = b"") -> tuple[bytes, bytes]:
        ...
        for i in range(0, len(plaintext), bsv * 16):
            ...
            for chunk, block, checksum in zip(chunks, blocks, checksums, strict=True):
    ...
    def decrypt(self, ciphertext: bytes, tag: bytes, nonce: bytes, header: bytes = b"") -> bytes:
        ...
        for i in range(0, len(ciphertext), bsv * 16):
            ...
            for _, block, checksum in zip(chunks, blocks, checksums, strict=True):
    ...
    def compute_auth_tag(
        ...
        out = []
        for b1, b2 in zip(plaintext_checksum, header_checksum, strict=True):  # type: int, int
            out.append(b1 ^ b2)
        return bytes(out)
    ...
    def compute_header_checksum(self, keygen: BlakeKeyGen, header: bytes, counter: int) -> bytes:
        ...
        for i in range(0, len(header), bsv * 16):
            ...
            for _chunk, block, checksum in zip(chunks, blocks, checksums, strict=True):
                checksum.xor_with(block.state)
            counter += bsv
