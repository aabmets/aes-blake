...
def ent_g(x, biascorrect=True):
    ...
    ln2 = np.log(2)
    if biascorrect:
        psiterms = sp.special.psi((Ntrl - np.arange(1,Nvarx+1).astype(np.float64))/2.0) / 2.0
...
def mi_model_gd(x, y, Ym, biascorrect=True, demeaned=False):
    ...
    if biascorrect:
        ...
        psiterms = sp.special.psi((Ntrl - vars).astype(np.float64)/2.0) / 2.0
        dterm = (ln2 - np.log(float(Ntrl-1))) / 2.0
        Hunc = Hunc - Nvarx*dterm - psiterms.sum()

        dterm = (ln2 - np.log((Ntrl_y-1).astype(np.float64))) / 2.0
        psiterms = np.zeros(Ym)
        for vi in vars:
            idx = Ntrl_y-vi
            psiterms = psiterms + sp.special.psi(idx.astype(np.float64)/2.0)
...
def gcmi_mixture_cd(x,y,Ym):
    ...
    for yi in range(Ym):
        ...
        ydat.append(yi*np.ones(xm.shape[1],dtype=np.int64))
...
def cmi_ggg(x, y, z, biascorrect=True, demeaned=False):
    ...
    ln2 = np.log(2)
    if biascorrect:
        psiterms = sp.special.psi((Ntrl - np.arange(1,Nvarxyz+1)).astype(np.float64)/2.0) / 2.0
        dterm = (ln2 - np.log(Ntrl-1.0)) / 2.0
